...
- (void)setUp {
  ...
  [targetApp executeSyncWithBlock:^{
    [FTRSyncAPITest openTestViewNamed:@"Basic Views"];
  }];
}
...
- (void)testPushAndPopRunLoopModes {
  [targetApp executeSyncWithBlock:^{
    ...
    [[UIApplication sharedApplication] pushRunLoopMode:@"Boo" requester:[FTRSyncAPITest class]];
    ...
    GREYAssertEqualObjects([[UIApplication sharedApplication] grey_activeRunLoopMode], @"Boo",
                           @"should be equal");
    [[UIApplication sharedApplication] popRunLoopMode:@"Boo" requester:[FTRSyncAPITest class]];
